#Degree-based Sparsification: Display only nodes that have a certain minimum degree (number of connections) in the graph. Nodes with a lower degree can be filtered out, reducing the visual clutter and focusing on the more connected nodes.
...
def compute_metrics(traces,outputdir):
    ...
    generatehtml(sorted_d,outputdir,threshold=0.5)
...
def generatehtml(data, outputdir, threshold):
    # Calculate node degrees
    degrees = np.zeros(len(data), dtype=int)
    for i, (k, v) in enumerate(data.items()):
        degrees[i] = v[0] + v[1] + v[2] + v[3]

    # Sort nodes based on degrees in descending order
    sorted_indices = np.argsort(degrees)[::-1]
    sorted_data = {list(data.keys())[i]: data[list(data.keys())[i]] for i in sorted_indices}

    # Apply sparsification based on the threshold
    num_nodes = int(threshold * len(sorted_data))
    sparsified_data = dict(list(sorted_data.items())[:num_nodes])

    # Initialize Directed graph
    g = graphviz.Digraph('G', filename='tmp_gh.gv', format="svg")

    # Add each service, operation pair as a node to the graph
    for k, v in sparsified_data.items():
    ...
    for k, v in sparsified_data.items():
        for n, val in v[4].items():
            if n in sparsified_data:  # Check if the target node is also sparsified
